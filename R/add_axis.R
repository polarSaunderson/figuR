add_axis <- function(axis,
  ...
  #'
  #' @param meshlines numeric: How many lines is the scaffold made up of?
  ...
  #'   'meshlines'.
  #'
  #' @param alignMidPoints BINARY: Should the values be aligned midway between
  ...
  # meshlines (preference is that order).
  if (is.null(labels)) {
  ...
  if (isFALSE(alignMidPoints)) {       # ticks on the values (for line graph)
    ...
    # Option 2: Ticks should be offset (i.e. for a matrix or bar chart)
    if (!is.null(interval)) {
